...
module.exports = async (req, res) => {
    ...
    try {
        if (req.method === 'GET') {
            ...
            const rows = await neonQuery('SELECT * FROM subscribers ORDER BY subscribed_at DESC');
        ...
        if (req.method === 'DELETE') {
            ...
            const sub = await neonQuery('SELECT id FROM subscribers WHERE email = $1', [email.toLowerCase()]);
            await neonQuery(
