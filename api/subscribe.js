...
module.exports = async (req, res) => {
    ...
    if (!apiKey) {
        return res.status(503).json({
            ...
            hint: 'Ajoutez LOOPS_API_KEY dans les variables d\'environnement Vercel.'
        });
    }

    try {
        const result = await loopsSubscribe(email, source || 'Team Group', apiKey);
        ...
        console.error('Loops API error:', err.message);
        return res.status(500).json({ error: 'Erreur serveur. Réessayez plus tard.' });
    }
};

function loopsSubscribe(email, source, apiKey) {
            ...
            response.on('end', () => {
                try {
                    ...
                    if (response.statusCode === 200 && parsed.success) {
                        ...
                        resolve({
                            success: false,
                            message: parsed.message || `Erreur ${response.statusCode}`
                        });
                    }
                } catch (e) {
                    resolve({
